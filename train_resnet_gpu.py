"""
CRAIG Training Script for GPU (Backend.AI)
Modified for GPU execution with CUDA support
"""
import argparse
import os
import time
...
parser = argparse.ArgumentParser(description='CRAIG ResNets for CIFAR10 - GPU Version')
...
parser.add_argument('--save-every', dest='save_every',
                    ...
                    type=int, default=50)
parser.add_argument('--gpu', default='0', type=str, help='GPU ID to use (e.g., "0" or "0,1" for multi-GPU)')
...
parser.add_argument('--random_subset_size', '-rs', type=float, help='size of the random subset', default=1.0)
...
parser.add_argument('--ig', type=str, help='method', default='sgd', choices=['sgd', 'adam', 'adagrad'])
...
parser.add_argument('--cluster_features', '-cf', dest='cluster_features', action='store_true', help='cluster features')
parser.add_argument('--cluster_all', '-ca', dest='cluster_all', action='store_true', help='cluster all')
parser.add_argument('--start-subset', '-st', default=0, type=int, metavar='N', help='start subset selection epoch')
parser.add_argument('--save_subset', dest='save_subset', action='store_true', help='save subset indices')
parser.add_argument('--no-cuda', dest='no_cuda', action='store_true', help='disable CUDA even if available')
...
def setup_device(no_cuda=False):
    """Setup CUDA device"""
    global device
    if no_cuda or not torch.cuda.is_available():
        device = torch.device('cpu')
        print("⚠️  Using CPU (CUDA not available or disabled)")
    else:
...
# Define IndexedDataset at module level for pickling
class IndexedDataset(torch.utils.data.Dataset):
    def __init__(self, root='./data', transform=None):
        self.cifar10 = datasets.CIFAR10(root=root, train=True, transform=transform, download=True)
...
def main(subset_size=.1, greedy=0):
    ...
    print(f'\n--------- subset_size: {subset_size}, method: {args.ig}, moment: {args.momentum}, '
    ...
    # Create model
    if torch.cuda.is_available() and not args.no_cuda and len(args.gpu.split(',')) > 1:
        # Multi-GPU
        model = torch.nn.DataParallel(resnet.__dict__[args.arch]()).cuda()
        print(f"�� Using {len(args.gpu.split(','))} GPUs")
    else:
        # Single GPU or CPU
    ...
    train_loader__ = torch.utils.data.DataLoader(
        ...
        num_workers=args.workers, pin_memory=torch.cuda.is_available() and not args.no_cuda)

    indexed_dataset = IndexedDataset(root='./data', transform=transforms.Compose([
        transforms.RandomHorizontalFlip(),
        transforms.RandomCrop(32, 4),
        transforms.ToTensor(),
        normalize,
    ...
    # Loss functions
    train_criterion = nn.CrossEntropyLoss(reduction='none').to(device)
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if subset_size >= 1 or epoch < args.start_subset:
                ...
                if greedy == 0:
                    ...
                    print(f'�� Selecting {B} elements from random subset of size: {len(subset)}')
                else:
                    ...
                    print(f'⏱️  FL time: {ordering_time:.3f}s, Sim time: {similarity_time:.3f}s')
                    grd_time[run, epoch], sim_time[run, epoch] = ordering_time, similarity_time

                times_selected[run][subset] += 1
                coverage = np.sum(times_selected[run] == 0) / len(times_selected[run]) * 100
                print(f'�� {coverage:.3f}% not selected yet')
                not_selected[run, epoch] = coverage
            ...
            if epoch > 0 and epoch % args.save_every == 0:
                ...
                }, is_best, filename=os.path.join(args.save_dir, 'checkpoint_epoch{}.pth'.format(epoch)))

            print(f'�� Run: {run}, Epoch: {epoch}, Prec@1: {prec1:.3f}, Loss: {tl:.3f}, '
                  f'Best: {best_prec1:.3f}, Not selected: {not_selected[run][epoch]:.3f}%')
            ...
            if args.save_subset:
                np.savez(f'{folder}_{args.ig}_moment_{args.momentum}_{args.arch}_{subset_size}'
                         f'_greedy_{greedy}_{args.lr_schedule}_lag_{args.lag}_subset',
                ...
                np.savez(f'{folder}_{args.ig}_moment_{args.momentum}_{args.arch}_{subset_size}'
                         f'_greedy_{greedy}_{args.lr_schedule}_lag_{args.lag}',
                         train_loss=train_loss, test_acc=test_acc, train_acc=train_acc, test_loss=test_loss,
                         data_time=data_time, train_time=train_time, grd_time=grd_time, sim_time=sim_time,
    ...
    print(f'\n�� Final Results:')
    print(f'   Max Test Accuracy: {np.max(test_acc, 1)}, Mean: {np.mean(np.max(test_acc, 1)):.3f}')
    print(f'   Min Not Selected: {np.min(not_selected, 1)}, Mean: {np.mean(np.min(not_selected, 1)):.3f}')


def train(train_loader, model, criterion, optimizer, epoch, weight=None):
    """Run one train epoch"""
    if weight is None:
        weight = torch.ones(TRAIN_NUM).to(device)
