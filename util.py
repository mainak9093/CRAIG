...
# --- Portability note (Kaggle / public GPU) ---
# `nearpy` is imported below in the original code but is never actually used in
# this file, and it is not installed on Kaggle. The imports are commented out so
# the module loads on Kaggle. Uncomment them for the original backend-GPU setup.
...
# TensorFlow imports disabled for Python 3.14 compatibility
